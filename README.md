...
- Polymorphic Lookup Relationship: Special type of lookup field on Task/Event object that can link a child record to more than one type of parent object using WhoID (Contact or a Lead) and WhatID (Account, Campaign, Case, or Opportunity). <br>
...
We can create 1000 permission sets, One permission set can be assigned to 1000 users only.
...
A dashboard is a visual representation of salesforce data from one or more salesforce reports that helps users to identify trends and patterns to make informed business decisions. We can add 25 different components in dashboard.
...
Territory management is a feature that allows you to grant access to accounts and opportunities based on criteria such as geography, territory, industry, product line, or customer size. Only one territory model can be active at any given time.
